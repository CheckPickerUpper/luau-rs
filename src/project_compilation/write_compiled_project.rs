...
pub fn write_compiled_project_atomically(
    ...
    let (staging_directory, backup_directory) = match publication_paths(output_directory) {
        Ok(paths) => paths,
    ...
    if staging_directory.exists() || backup_directory.exists() {
        return Err(rejection((
            output_directory,
    ...
    let destination_was_present = output_directory.exists();
    if destination_was_present {
        if let Err(error) = fs::rename(output_directory, &backup_directory) {
            let _ = fs::remove_dir_all(&staging_directory);
            return Err(rejection((
                output_directory,
                ProjectOutputOperation::MovePreviousOutput,
                error.kind(),
            )));
        }
    }
    if let Err(error) = fs::rename(&staging_directory, output_directory) {
        let restore_error = if destination_was_present {
            fs::rename(&backup_directory, output_directory).err()
        } else {
            None
        };
        let _ = fs::remove_dir_all(&staging_directory);
        if let Some(restore_error) = restore_error {
            return Err(rejection((
                output_directory,
                ProjectOutputOperation::RestorePreviousOutput,
                restore_error.kind(),
            )));
        }
        return Err(rejection((
...
fn publication_paths(
...
) -> Result<(PathBuf, PathBuf), ProjectOutputRejection> {
    ...
    Ok((
        parent_directory.join(format!(".{directory_name}.luau-rs-staging-{process_id}")),
        parent_directory.join(format!(".{directory_name}.luau-rs-backup-{process_id}")),
    ))
}
